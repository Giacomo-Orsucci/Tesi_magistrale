...
This specific parametrization reflects the intrinsic difficulty of the spatial embedding problem. The initial temperature ($T_{INIT}$) must be sufficiently large to "melt" any initial configuration that severely violates the $d_{min}$ constraint (which generates massive penalty values). Concurrently, the combination of a high cooling rate ($0.98$) and a substantial number of steps per level ($800$) ensures that the algorithm meticulously sweeps the continuous coordinate space before freezing. As with the GA, these parameters act as the architectural baseline and can be dynamically tuned to tackle instances of varying density and dimensionality.
...
\section{Compaison with 10x10 instance}

\section{15x15: GAs rupture}

%valutare quali altri mettere di scala superiore.