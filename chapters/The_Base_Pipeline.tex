...
This chapter presents the baseline architecture of our embedding pipeline and it precedes the related experimental results. The purpose is to detail how the embedding problem was methodologically approached, which embedding optimization methods were selected and tested, and what scalability limitations  and fitness scores emerged. Finally, it will be discussed how these limitations are natively imposed by the interplay between the mathematical formulation and the specific hardware constraints of the target quantum platform.
...
\begin{itemize}
    ...
    \item \textbf{Global Channel Energetics and Scaling:} The QPU operates via a single \texttt{rydberg\_global} channel. This imposes ceilings on the maximum allowable Rabi frequency ($\Omega_{max}$) and the maximum absolute detuning ($|\delta|_{max}$). Because raw QUBO weights can mathematically exceed these physical capacities, the embedding pipeline must calculate a restrictive \textit{Scale Factor}. As implemented in the codebase \cite{orsucci_qubo_ga_quantum}, the entire QUBO matrix is dynamically (instance per instance) scaled down by a factor of:
    \begin{equation*}
        \min \left( \frac{V_{max}}{\max(Q_{\text{off-diagonal}})}, \frac{|\delta|_{max}}{\text{avg}(Q_{\text{diagonal}})} \right)
    \end{equation*}
    This formulation accounts for both the maximum inter-atomic repulsive interaction and the single-atom driving energy, deliberately selecting the most restrictive ratio (the minimum) to guarantee that the scaled problem strictly respects all physical hardware limits simultaneously.
\end{itemize}
...
To accurately evaluate the topological mismatch between the physical layout and the mathematical abstraction, we partition the upper-triangular interactions into two disjoint sets. The active edges $E_{\text{active}} = \{(i,j) \mid |Q_{ij}| > \epsilon\}$ represent the necessary logical connections, while the inactive edges $E_{\text{inactive}} = \{(i,j) \mid |Q_{ij}| \le \epsilon\}$ represent pairs that must not physically interact. A tolerance of $\epsilon = 10^{-5}$ is introduced to safely handle floating-point inaccuracies. This explicit classification ensures that the algorithm can accurately shape the required couplings while actively filtering out the noise.
Consequently, the topological base error ($E_{base}$) is computed by measuring the exact deviation on the active bonds, while simultaneously applying a heavy penalty for any unwanted crosstalk on the inactive ones:
...
\begin{itemize}
    ...
    \item \textbf{Dynamic Early Stopping:} Because the embedding is executed inside a multi-start loop (e.g., $10$ independent restarts), waiting for all runs to finish can incur unnecessary classical overhead if a good enough embedding is discovered early. An early stopping condition was introduced by calculating a dynamic energy target empirically determined ($E_{target} = N \times 12.0$). If any specific SA restart achieves an energy score strictly below this threshold, the algorithm assumes the layout is physically optimal for analog quantum execution, immediately halting any remaining scheduled restarts.
\end{itemize}
