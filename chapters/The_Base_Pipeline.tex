...
\section{Alternative Embedding: SA}
%GIUSTO PER DARE TRACCIA SU CUI CONTINUARE:

%Da valutare quanto approfondire e sicuramente da ricontrollare bene il collegamente con il prima ed il dopo.

%Here we explain and present in brevity an alternative embedding approach using Simulated Annealing. Numerous publications use Simulated Annealing to approximate energy solutions also as a benchmark for proposed quantum algorithm like in \cite{} and in \cite{das2025grid}, works that inspired the idea to
%use SA, instead as a benchmark or as an approximation, to  find a feasible embedding for our QUBO problems. Moreover, as we will see in the following \cite{das2025grid} inspired the idea to try a divide et impera approach.
