...
The aim is to explain the inherent complexity of finding efficient embeddings. We will start by introducing basic concepts 
...
\begin{tikzpicture}[scale=1.5]
    ...
    % Definisco i nodi (punti)
    \coordinate (A) at (0, 0);
    \coordinate (B) at (1.5, 0.5);
    \coordinate (C) at (0.5, 1.5);
    \coordinate (D) at (2.5, 1.2);

    % Disegno gli archi validi (distanza <= r)
    \draw[thick] (A) -- (B) -- (C) -- (A);
    \draw[thick] (B) -- (D);
    % Disegno un arco non valido per mostrare che la distanza > r
    ...
    \end{scope}
\end{tikzpicture}
\caption{Equivalent geometric representations of a Unit Disk Graph (UDG). \textbf{(a)} The Distance Model connects nodes $u$ and $v$ if their Euclidean distance is $d \leq r$. \textbf{(b)} The Intersection Model places a disk of radius $r/2$ centered on each node; edges are formed exclusively where disks overlap.}
...
Consequently, the core mathematical constraint of the MIS problem, which strictly forbids selecting two adjacent vertices, is natively and physically enforced by the Rydberg blockade phenomenon, which prevents any two atoms within a distance $R_b$ from being simultaneously excited to the Rydberg state. Because of this perfect theoretical alignment, applying a global laser pulse designed to maximize the number of excited atoms in the 2D register forces the quantum many-body system to naturally evolve toward a ground state that inherently encodes the exact solution to the Maximum Independent Set of the corresponding geometric graph \cite{Henriet2020quantumcomputing}.
\begin{figure}[htbp]
\centering
...
\end{tikzpicture}
\caption{Physical realization of the Maximum Independent Set (MIS) via the Rydberg blockade. Red dots represent atoms excited to the Rydberg state (\textbf{MIS selected nodes}), while black dots are atoms in the ground state (\textbf{unselected nodes}). The shaded red regions represent the Rydberg blockade volume (radius $R_b$). The physical mechanism preventing two Rydberg excitations from overlapping perfectly enforces the mathematical constraint of the MIS problem.}
...
Crucially for this thesis, several recent studies have started exploring problem partitioning as a workaround for hardware scale limitations. Works by Nembrini et al. \cite{nembrini2024application, Nembrini_2026} and Das et al. \cite{das2025grid} propose dividing the original combinatorial problem into sub-instances resolved using localized arrays. While their specific methodologies differ from our scope, these partitioning strategies served as the foundational inspiration for the hybrid \textit{divide et impera} (divide and conquer) architecture proposed in the subsequent chapters of this work.
...
%continuare come è fatta Q nel nostro caso e perchè. Il dettaglio su x lo metterei direttamente nella pipeline

%Scaletta delle prossime cose da fare:

%Descrizione QUBO problems e magari specificare meglio perchè ci concentriamo sugli UDG like (o vedere se farlo nelle sezioni dopo)
%Valutare se mettere esempio di matrice che useremo o comunque di problema QUBO
%rileggere e valutare tutto il capitolo
%attaccare il capitolo 3
%valutare quando attaccare con sperimentazioni ulteriori e richieste